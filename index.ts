...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    ...
    async function getEnvironment(spaceId: string, environmentId: string) {
    ...
    switch (name) {
      ...
      case "update_asset": {
        ...
        const environment = await getEnvironment(spaceId, environmentId);
        const asset = await environment.getAsset(assetId);
        if (title) asset.fields.title = { 'en-US': title };
        if (description) asset.fields.description = { 'en-US': description };
        if (file) asset.fields.file = { 'en-US': file };
